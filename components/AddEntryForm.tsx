...
export default function AddEntryForm({
  ...
  const formSchema = z.object({
    ...
    perpetrators: z
      .string()
      .refine((v) => v.split(",").every((name) => name.trim().length > 0), {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    const entry: Omit<JournalEntry, "category" | "organization"> & {
      ...
      perpetrators: values.perpetrators ? values.perpetrators.split(",") : [],
      witnesses: values.witnesses ? values.witnesses.split(",") : [],
    };
